...
const Shader = function(gl, shaderType, sourceFileName) {
  ...
  if (Shader.source.hasOwnProperty(sourceFileName)) {
    ...
    throw new Error(
      "Shader " +
        sourceFileName +
        " not found. Check spelling, and whether the essl file is embedded into the html file."
    );
  }
  ...
  if (!gl.getShaderParameter(this.glShader, gl.COMPILE_STATUS)) {
    console.log(
      "Error in shader " +
        ...
        ":\n" +
        gl
          .getShaderInfoLog(this.glShader)
          .replace(/ERROR: 0/g, Shader.sourcePathURL + sourceFileName)
    );
    throw new Error("Shader " + sourceFileName + " had compilation errors.");
  }
};

Shader.sourcePathURL =
  document.currentScript.src.split("Shader.js")[0] + "shaders/";
Shader.source = {};
